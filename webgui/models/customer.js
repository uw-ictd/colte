    ...
    function transfer_func(trx) {
      return fetch_bals(trx).then((data) => {
        ...
        .then((data2) => {
          ...
          fs.appendFile(transaction_log, dateTime() + " USERTRANSFER " + sender_imsi + " " + receiver_imsi + " " + amount + "\n", function(err) {
              if(err) {
                  return console.log(err);
              }
          });
  ...
  purchase_package(imsi, cost, data) {
    ...
    function purchase_func(trx) {
      ...
      .then(function(rows) {
        ...
        var rval = knex.update({balance: newBalance, data_balance: newData}).where('imsi', imsi).from('customers').transacting(trx).then(trx.commit, trx.rollback)
        .catch(function (error) {
          throw new Error(error.sqlMessage);
        });

        fs.appendFile(transaction_log, dateTime() + " PURCHASE " + imsi + " " + data + " " + cost + "\n", function(err) {
          if(err) {
              return console.log(err);
          }
        });
