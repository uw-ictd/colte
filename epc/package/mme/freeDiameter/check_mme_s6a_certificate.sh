...
function main() {
  ...
  if [ $# -lt $EXPECTED_ARGS ]; then
    echo "Usage: check_mme_s6a_certificate output_path fqdn"
    return 1
  fi
  local output_path=$1
  local fqdn=$2
  if [ -d $output_path ]; then
    if [ -f $output_path/mme.cert.pem ];  then
      ...
      if [ a$full_hostname == a$fqdn ]; then
        echo "MME S6A: Found valid certificate in $output_path"
        return 0
      else 
        echo "Bad mme fqdn found in cert file: $full_hostname  fqdn is $fqdn"
      fi
    else
      echo "File $output_path/mme.cert.pem not found"
    fi
  else
    echo "Directory $output_path not found"
  fi
  echo "MME S6A: Did not find valid certificate in $output_path"
  echo "MME S6A: generating new certificate in $output_path..."
  _create_mme_certs $output_path $fqdn
  if [ $# -lt 3 ] ; then
    main $output_path $fqdn 2
    return $?
  else
    echo "Could not access to output path: $output_path"
    exit 1
  fi
}
