  ...
  if [ -d $output_path ]; then
    if [ -f $output_path/hss.cert.pem ];  then
      ...
      if [ a$full_hostname == a$fqdn ]; then
        echo "HSS S6A: Found valid certificate in $output_path"
        return 0
      else 
        echo "Bad hss fqdn found in cert file: $full_hostname  fqdn is $fqdn"
      fi
    fi
  fi
  echo "HSS S6A: Did not find valid certificate in $output_path"
  echo "HSS S6A: generating new certificate in $output_path..."
  _create_hss_certs $output_path $fqdn
  if [ $# -lt 3 ] ; then
    main $output_path $fqdn 2
    return $?
  else
    echo "Could not access to path: $output_path"
    exit 1
  fi
}
